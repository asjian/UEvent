...
import {View, Text, TouchableOpacity, StyleSheet, Image, Keyboard, Dimensions} from 'react-native';
...
export default function mapFilters({navigation,searchParams,visible}) {
    ...
    const clearFilters = () => {
        if(catlength != 0 || timeFilterValue != 'Anytime') {
            ...
            searchParams.TimeRange = {startDate:'',endDate:'',startTime:'',endTime:'',value:'Anytime'};
        ...
        return (
            <View style = {styles.container}>
                <TouchableOpacity style = {[styles.filterButton,{backgroundColor:timeFilterValue=='Anytime'?'white':'#0085ff'}]} onPress = {() => pressHandler('DateRange')}>
                    <Image source = {require('../assets/clock.png')} style = {[styles.icon,{tintColor:showDefault||timeFilterValue=='Anytime'?'black':'white'}]}/>

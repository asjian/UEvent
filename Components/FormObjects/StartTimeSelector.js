...
export const StartTimeSelector = ({onChangeFormik, realStartDate, realEndDate}) => {
  ...
  const onChange = (event, selectedDate) => {
    ...
    onChangeFormik('StartTimeFormPurposes', currentDate);
    onChangeFormik('RealStartDateTime', new Date(new Date(realStartDate).setHours(currentDate.getHours(),currentDate.getMinutes(),currentDate.getSeconds(), currentDate.getMilliseconds())));
    ...
    <View>
        <View style={{marginLeft: 20, marginTop: 10}}>
            <TouchableOpacity onPress = {showTimepicker}>
                <Text style={styles.dateText}>{realStartDate.toLocaleString('en-US', { hour: 'numeric',minute: '2-digit', hour12: true })}</Text>
            </TouchableOpacity>
        </View>
